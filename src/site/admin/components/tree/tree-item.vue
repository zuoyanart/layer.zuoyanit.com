...
<div>
    <li  :id="items.id" :path="items.path">
        ...
        <em>[{{items.weight}}]{{items.name}}</em>
        <span>
          <router-link :to="'/tree/edit/?id='+items.id">编辑</router-link>
          <router-link :to="'/tree/add/?pid='+items.id">添加子节点</router-link>
...
export default {
  ...
  methods: {
    ...
    hideNode: async function (nodeid, event) {
      ...
      const ishide = o.innerHTML === '隐藏' ? 1 : 0;
      await this.$tools.httpAgent(
        '/tree/ishide',
        'post',
        'id=' + nodeid + '&ishide=' + ishide
      );
