...
const tools = function (layer, config = {}) {
  const self = {};
  // sleep
  ...
  self.formatParams = function (data) {
    if (typeof (data) !== 'string') {
      ...
      arr.push('timestamp=' + this.getUnixTime());
      return arr.join('&');
    } else {
      if (data === '') {
        data += 'a=b';
      }
      return data + '&timestamp=' + this.getUnixTime();
    }
  };
  // 获取签名
  self.getSign = (data, key = 'kG8by9pl^d') => {
    let params = data;
    if (typeof (data) === 'string') {
      params = data.split('&');
    }
    ...
    sign = encodeURIComponent(encodeURIComponent(sign));
    return sign;
  };
  /**
   * ajax请求
  ...
  self.httpAgent = (url, method = 'get', data = '', option = {}) => {
    ...
    data = self.formatParams(data);

    let sign = '';
    if (option.login === true) {
      const jwt = JSON.parse(localStorage.getItem('jwt') || '{}');
      data += '&userkey=' + jwt.userkey;
      sign = self.getSign(data, jwt.usersecret);
    } else {
      data += '&userkey=' + config.userkey.key;
      sign = self.getSign(data, config.userkey.secret);
    }
    ...
    let layerid = 0;
    if (layer.toast) {
      layerid = layer.loading.open('');
    } else {
      layerid = layer.loading(1);
    }
